...
from app.services.product_readiness import onboarding_status
...
def interview(
    ...
    user: User = Depends(RequireCredits("interview_prep")),
):
...
def roadmap(
    payload: dict,
    db: Session = Depends(get_db),
    user: User = Depends(RequireCredits("roadmap")),
):
    jobs = db.query(Job).order_by(Job.created_at.desc()).limit(100).all()
    gaps_payload = skill_gaps(user.profile_json or {}, [{"skills": job.skills_json or []} for job in jobs])
    target_role = payload.get("target_role") or "AI Engineer"
    duration = payload.get("duration") or "8 weeks"
    return build_roadmap(
        target_role,
        user.profile_json or {},
        gaps_payload.get("missing_skills", []),
        duration=duration,
    )
...
def tailor(
    payload: dict,
    user: User = Depends(RequireCredits("tailor_resume")),
):
    return tailor_resume(user.resume_text or "", payload.get("job") or {}, user.profile_json or {})


@router.get("/onboarding")
def onboarding(user: User = Depends(get_current_user)):
