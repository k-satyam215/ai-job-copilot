...
@router.post("/answers")
async def answers(payload: CopilotAnswersRequest, user: User = Depends(get_current_user)):
    memory = build_profile_memory(user)
    results = await reason_answers([field.model_dump() for field in payload.fields], memory, payload.job)
    return {"answers": results, "profile_ready": bool(user.profile_json), "source": payload.source}
